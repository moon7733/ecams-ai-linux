...
4. 고객사가 없거나 연결 repo가 없으면 광주은행 repo 전체를 샘플로 검색한다.
5. 명시적인 `repo` 요청도 등록된 repo인지 확인하고 고객사가 함께 오면 소유 관계를 검증한다.
6. pms-bridge는 소스를 직접 읽지 않고 ecams-ai 내부 검색 API를 서비스 토큰으로 호출한다.
7. 검색 결과는 `entityId`, `kind`, `name`, `paths`, `score`만 반환한다.
