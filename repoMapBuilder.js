...
// .repoignore 로드
const repoIgnoreSet = new Set();
const repoIgnorePath = path.join(ROOT, '.repoignore');
if (fs.existsSync(repoIgnorePath)) {
  try {
    const lines = fs.readFileSync(repoIgnorePath, 'utf8').split('\n');
    for (let line of lines) {
      line = line.trim();
      if (line && !line.startsWith('#')) {
        // 경로 구분자 정규화
        repoIgnoreSet.add(line.replace(/\\/g, '/'));
      }
    }
    console.log(`[RepoMap] Loaded ${repoIgnoreSet.size} entries from .repoignore`);
  } catch (e) {
    console.error(`[RepoMap] Failed to load .repoignore: ${e.message}`);
  }
}
...
function walkFiles(base) {
  ...
  while (stack.length && out.length < MAX_FILES_SCAN) {
    ...
    if (stat.isDirectory()) {
      ...
      // .repoignore 필터링
      const relPath = path.relative(base, cur).replace(/\\/g, '/');
      if (repoIgnoreSet.has(relPath)) continue;
...
function extractTreeSitter(file, parser) {
  let content;
  try { content = smartRead(file); } catch (_) { return { defs: [], refs: [] }; }
...
function buildRepoMap(repoPath, message, tokenBudget) {
  ...
  let targetFile = null;
  const targetMatch = message.match(/\[타깃 파일: ([^\]]+)\]/);
  if (targetMatch) {
    targetFile = targetMatch[1].toLowerCase();
  }
  ...
  graph.forEachNode((node, attrs) => {
    ...
    if (attrs.type === 'symbol') {
      ...
      if (targetFile && fileLower.includes(targetFile)) {
        weight = Math.max(weight, 10000.0);
        hasKeywordMatch = true;
      } else {
        for (const kw of keywordSet) {
  ...
  // 파일 노드의 가중치를 자식 심볼들에게 전파 (triageTarget 등으로 파일명이 매칭된 경우 심볼들이 수혜를 받도록)
  graph.forEachNode((node, attrs) => {
    if (attrs.type === 'symbol' && personalization[attrs.file]) {
      personalization[node] = Math.max(personalization[node] || 0, personalization[attrs.file] * 0.9);
    }
  });
